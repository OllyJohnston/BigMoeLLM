...
- **2026-09-13: Staggered Expert Projection Streaming (engine 0.32.0)**: On `--moe-stream --overlap`, qwen4exp expert batches now publish the projections the FFN graph consumes first (`ffn_up_exps` before `ffn_gate_exps`) as wave one and defer `ffn_down_exps` to wave two, so GEMM1 + SwiGLU compute overlaps the down transfer. This fixes the historical two-wave emitting only the first recipe slot (gate) while the ready hook actually blocks on `up` first — the wave was a synchronous barrier. Gated strictly to `qwen4exp`; dense archs and the draft/MTP context bypass it; synchronization stays in the existing host-side `ready_` flags (no new sync, graph splits bit-identical). Measured on the reference Flash-Next harness: decode 10.1-10.5 → **10.97 tok/s** (~4-8%), `moe-overlap stall` 11 → 10 ms. The residual 10 ms is a physical NVMe bandwidth boundary (47.8 MiB/token at 591 MiB/s ≈ 81 ms transfer vs 80 ms compute), not a software stall — 8 I/O threads drop throughput to 402 MiB/s via drive queue saturation. The 4.5 ms stall target is documented as unreachable for this transfer volume; remaining tail is operational tuning (O_DIRECT, larger `--cache-mb`), not pipeline structure. Gates 16/16, zero TDR.
- **2026-09-06: Upstream Sync — Fast MoE GEMM Dispatch & Hadamard Guard (`95df4f50` submodule, engine 0.29.0)**: Two upstream cherry-picks landed on `bmoe/expert-ready-hook`. PR #27978 generalises `mm_ids_helper`'s lane padding to the next power of two, so `n_expert_used = 10` (Qwen3.8-Flash-Next's routed expert count) dispatches the fused multi-expert GEMM path (`case 10`) instead of the generic per-token warp-reduction fallback — MoE prefill-only, measured upstream 2334 → 2600 t/s on Flash-Next at 55k context. PR #27967 guards the Hadamard `k_rot` copy against an unassigned buffer so context shifting an unquantized K cache (`-ctk f32`) no longer crashes. Rejected from the same audit: PR #27970 (sparse-FA, conflicts with our QSA gather path) and PR #28040/#27991 (dependency / no impact). Verified: `mmid` dispatch reaches case 10, MUL_MAT_ID 880/880 + fusion 900/900 + topk 416/416 on CUDA0, gates 16/16, live Flash-Next `-ngl 42` decode at 10.8 tok/s with MTP 78.5% acceptance.
- **2026-09-13: Native Blackwell NVFP4 Acceleration Switch (engine 0.31.0)**: `--cuda-nvfp4 auto|on|off` exposes the fork's native E2M1 MMA path behind an architecture gate. `auto` (default) enables only for dense Qwen3.5 (`qwen35`) on sm_120+; `on` warns and falls back for any other arch; `off` never authorizes it. The resolution is logged (`Blackwell NVFP4 acceleration: ENABLED/DISABLED (arch: ..., cc: ...)`) and is inert to graph construction, so Flash-Next's zero-split capture is untouched (split counts bit-identical with and without the flag). Verified: Flash-Next logs `DISABLED (arch: qwen4exp, cc: 120)` with MTP intact; dense APEX runs unmodified; a real 448-NVFP4-tensor GGUF (esatapedico Qwen3.8-27B NVFP4 MID-HIGH) logs `ENABLED (arch: qwen35, cc: 120)` and decodes 357-544 tok/s with coherent output, peak 15.9 GB dedicated VRAM, zero shared spill, zero TDR. Gates 16/16.
- **2026-09-13: Block-Granular Adaptive KV Streaming (`2f598a90` submodule, engine 0.30.0)**: Transplanted the adaptive-kv-streaming implementation onto `bmoe/expert-ready-hook`, gated to `qwen35` dense models. `--kv-stream-stage-mib N` pages K/V through a host pool (`cudaHostAllocMapped`) into a small device ring, prefetched cross-layer on a dedicated copy stream inside the CUDA graph - the KV cache no longer has to fit in VRAM. Qwen3.8-27B at `-c 131072` decodes at **30.5 tok/s** through `CUDA_KV_Stream_Host` staging (vs ~8 tok/s unstreamed), VRAM avg 11.2 GB / peak 15.5 GB on 16 GB, zero WDDM paging / zero TDR. MoE untouched: Flash-Next (`qwen4exp`) bypasses streaming and its zero-split capture + `--cpu-moe` detached-MTP stay green; MTP/draft contexts never stream.
- **2026-09-05: Pinned Layers Run Native CUDA Again Under `--cpu-moe` (`5d00e641` submodule, engine 0.27.4)**: Hybrid static offload no longer forces pinned layers `0..n_pinned-1` onto the CPU buffer type. With a pass-1 scheduler guard in the fork (`ggml_backend_sched_backend_id_from_cur` — the *only* scheduler delta; pass-3 untouched), any op whose weight lives in a non-host buffer is pinned to the owning backend instead of falling through to a CPU worker that would dereference a device pointer as host memory (`movzx` access violation). Pinned layers once again bind to the CUDA0 device buft at `-ngl n>0` and dispatch MoE nodes to the GPU; `n_pinned..255` keep the plain-CPU override and stream from host. Decode on the 35B-A3B reference profile returns to the **50–56 tok/s** bracket (~5 tok/s on the faulting intermediate), private commit holds ~14.5 GB at `-c 32000`, KV-reuse runs hit 55 tok/s with 84.85% spec acceptance. Gates 16/16; `--n-pinned-layers 0` / `-ngl 0` unchanged.
- **2026-09-05: Detached MTP Draft Heads (BMOE-SPEC-06, engine 0.28.0)**: `--model-draft` / `-md` loads the trained NextN head from its own gguf (Qwen3.8-Flash-Next `mtp-*.gguf` sidecars) with `--n-gpu-layers-draft` / `-ngld` offloading it independently of the target, and `--spec-type draft-mtp|draft` choosing between the NextN-head and a standalone autoregressive draft model. The head borrows the target's LM head when the export marks `nextn_shared_target_tensors`; the streamer appends the head file as an extra shard so its `blk.<n_layer>.nextn` experts stream too. Bundled the upstream PR #28123 recurrent-state rollback port: rejected drafts roll the target's conv/DeltaNet state back via `n_rs_seq` snapshots instead of serializing the whole recurrent state to host (`SEQ_RM_TYPE_FULL`) every turn. Verified live on Flash-Next with `-ngl 12 -ngld 99`: base + head offload telemetry, draft ctx `graph splits = 1`, 50% acceptance / 2.50 mean draft length, zero `SEQ_RM_TYPE_FULL`, gates 16/16.
